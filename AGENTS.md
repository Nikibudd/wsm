...
**Do not merge a feature branch into `develop` yourself.** Create the
feature branch, commit to it, and push it — but leave merging into
`develop` to the user, who does it manually via a PR (this is also what
`.github/workflows/ci.yml`'s PR-triggered build+test is for — it needs an
actual PR to run against). This is a deliberate change from earlier in this
project's history, where merging feature branches into `develop` directly
(no PR) was the norm — don't follow that older pattern now that it's been
